import { View, Text, StyleSheet, TouchableOpacity, FlatList, ActivityIndicator, TextInput, Platform } from 'react-native';
...
export default function RawatInapScreen() {
  ...
  useEffect(() => {
    if (!isRawatInapEnabled(Platform.OS)) {
      setLoading(false);
      setData([]);
      setFilteredData([]);
      setHasMore(false);
  ...
  const fetchPage = async (nextPage: number, options: { reset: boolean; query: string }) => {
    ...
    try {
      ...
      const response = await api.master.list('jns_perawatan_inap', params);
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          onEndReached={isRawatInapEnabled(Platform.OS) ? loadMore : undefined}
          ...
          ListEmptyComponent={() => (
            <View style={styles.centerContainer}>
              <Text style={styles.emptyText}>
                {isRawatInapEnabled(Platform.OS) ? 'Data tidak ditemukan.' : 'Fitur Rawat Inap tidak tersedia.'}
              </Text>
            </View>
          )}
        />
