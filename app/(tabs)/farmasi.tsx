...
export default function FarmasiScreen() {
  ...
  const fetchPage = async (nextPage: number, options: { reset: boolean; query: string }) => {
    ...
    try {
      ...
      const response = await api.master.list('databarang', params);
      ...
      setData((prev) => {
        ...
        return merged.filter((item: any) => {
          const key = String(item?.kode_brng ?? '');
          if (!key) return true;
  ...
  useEffect(() => {
    ...
    setFilteredData(
      (Array.isArray(data) ? data : []).filter((item: any) =>
        String(item?.nama_brng || '').toLowerCase().includes(keyword) ||
        String(item?.kode_brng || '').toLowerCase().includes(keyword)
      )
    );
  }, [data, search]);
