...
import { ChevronLeft, Search, FlaskConical, CreditCard, Layers } from 'lucide-react-native';
import { useEffect, useRef, useState } from 'react';
import { api } from '@/lib/api';
import { LinearGradient } from 'expo-linear-gradient';
import { BlurView } from 'expo-blur';

export default function LaboratoriumScreen() {
  const perPage = 20;
  ...
  const fetchPage = async (nextPage: number, options: { reset: boolean; query: string }) => {
    ...
    try {
      ...
      const response = await api.master.list('jns_perawatan_lab', params);
      ...
      console.error('Error fetching jns_perawatan_lab:', error);
      if (options.reset) {
        setData([]);
      }
    } finally {
    ...
    <TouchableOpacity style={styles.card}>
      <View style={styles.cardHeader}>
        <View style={styles.iconBox}>
          <FlaskConical size={20} color="#62B986" />
        </View>
      ...
      <View style={styles.cardFooter}>
        <View style={styles.infoCol}>
          ...
          <Text style={styles.infoLabel}>{item.nm_kategori || 'Kat. Lab'}</Text>
        </View>
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.headerTitle}>Layanan Laboratorium</Text>
      </LinearGradient>
